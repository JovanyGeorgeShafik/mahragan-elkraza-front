...
function Signup() {
    ...
    function submit(e:FormEvent<HTMLFormElement>){
        ...
        axios.post("http://localhost:3000/signup",formData).then(resp=>{setMessage("تمام!");axios.post("https://mahragan-elkraza-jovanygeorgeshafik5590-ldb3030b.leapcell.dev/api/jwt",{jwt:resp.data.jwt}).then(res=>res)}).catch(err=>setMessage("في مشكلة"));
   
      }
  return (
   <form className="flex flex-col gap-2" onSubmit={submit}>
  <h2 className="text-center font-bold text-3xl bg-gradient-to-bl from-teal-400 to-blue-500 bg-clip-text fill-transparent text-transparent">إنشاء حساب</h2>
  {
    message&&
  ...
  <button type="submit" className=" bg-gradient-to-bl from-teal-400 text-white font-bold to-blue-500 p-2 rounded-2xl w-fit mx-auto">إنشاء</button>
